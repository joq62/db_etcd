...
read(Key,DeplId)->
    Return=case read(DeplId) of
		   ...
		   case  Key of
			   ...
			   {ok,DeployInfo};
		       Err ->
			   {error,['Key eexists',Key,DeplId,?MODULE,?LINE]}
		   end
	   end,
    Return.
...
read_all() ->
    Z=do(qlc:q([X || X <- mnesia:table(?TABLE)])),
    [{Id,ApplName,PodsInfo,DeployInfo}||{?RECORD,Id,ApplName,PodsInfo,DeployInfo}<-Z].
...
read(Object)->
    ...
    Result=case Z of
		  ...
		  [];
	       [X]->
		   {X#?RECORD.deployment_id,X#?RECORD.appl_name,X#?RECORD.pods,X#?RECORD.deployment_info}
	   end,
    Result.
